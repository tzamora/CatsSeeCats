...
public class ScreenJoystick : MonoBehaviour
{
    ...
    */
	...
	private static List<ScreenJoystick> joysticks;
	...
	private void Awake()
	{
		...
		if (!enumeratedJoysticks)
		{
			try
			{
				...
				joysticks = new List<ScreenJoystick>(objs.Length);

				foreach (GameObject obj in objs)
				{
					ScreenJoystick newJoystick = obj.GetComponent<ScreenJoystick>();
		...
		else
		{
			for (int i = 0; i < count; i++)
			{
				...
				if (shouldLatchFinger && (lastFingerId == -1 || lastFingerId != touch.fingerId))
				{
					...
					foreach (ScreenJoystick j in joysticks)
					{
						if (j == this)
						{
							continue;
